...
data=fread("Project2\\kiwi_bubbles_P2.csv",stringsAsFactors = F)
demo=fread("Project2\\demo_P2.csv",stringsAsFactors = F)
...
beta1= coef(mle_noseg)["price"]
beta0KB = coef(mle_noseg)["KB:(intercept)"]
beta0KR = coef(mle_noseg)["KR:(intercept)"]
beta0MB = coef(mle_noseg)["MB:(intercept)"]
...
demand_KB_KR_MB=function(pKB,pKR,pMB,parameter_1){
    probability_KB=exp(parameter_1[1]+parameter_1[4]*pKB)/(1+exp(parameter_1[1]+parameter_1[4]*pKB)+exp(parameter_1[2]+parameter_1[4]*pKR)+exp(parameter_1[3]+parameter_1[4]*pMB))
    probability_KR=exp(parameter_1[2]+parameter_1[4]*pKR)/(1+exp(parameter_1[1]+parameter_1[4]*pKB)+exp(parameter_1[2]+parameter_1[4]*pKR)+exp(parameter_1[3]+parameter_1[4]*pMB))
    probability_MB=exp(parameter_1[3]+parameter_1[4]*pMB)/(1+exp(parameter_1[1]+parameter_1[4]*pKB)+exp(parameter_1[2]+parameter_1[4]*pKR)+exp(parameter_1[3]+parameter_1[4]*pMB))
...
scatterplot3d(pricespace[, 1], pricespace[, 2], profitmat,
              ...
              color = adjustcolor("red", alpha.f = 0.6), pch = 16)
